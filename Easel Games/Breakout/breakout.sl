...
State ::= (sBall: Ball(0), sPaddle: Paddle(0), bricks: Brick(1), clear: bool(0), dead: bool(0), dust: Shrapnel(1), expl: Explosion(1), time: int(0));
//-------------------------------------------------------------------------------------------
//Helpers
...
buildState(ba(0), p(0), br(1), c(0), d(0), u(1), e(1), t(0)) := (sBall: ba, bricks: br, sPaddle: p, clear: c, dead: d, dust: u, expl: e, time: t);
...
initialState := buildState(ballStart, paddleStart, brickStart, false, false, [], [], 0);
...
newState(I(0), S(0)) := 
	...
	in
		initialState when clickReset(I)
	else
...
paddleHelper(S(0), flag(0)) := 
	...
	in
		buildState(S.sBall, nextPaddle, S.bricks, S.clear, S.dead, S.dust, S.expl, S.time);
...
motionHelper(S(0)) :=
	let
		...
		nextBall := yFlip(xFlip(tempB)) when (vWallCollide(tempB) and hWallCollide(tempB))
			...
			tempB;
	in
		buildState(ballMove(nextBall), S.sPaddle, takeAway(S.bricks, deadBricks), S.clear, S.dead, shrapMove(S.dust) ++ join(newDust(deadBricks)), S.expl ++ explosion(brCenter(deadBricks), S.time), S.time);
		//deadBricks;
...
sHelper(S(0)) := buildState(S.sBall, S.sPaddle, S.bricks, S.clear, S.dead, shrapMove(S.dust), S.expl, S.time);
...
sClean(S(0)) :=
	...
	in
		buildState(S.sBall, S.sPaddle, S.bricks, S.clear, S.dead, remaining, S.expl, S.time + 1);
