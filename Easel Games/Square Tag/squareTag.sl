...
Try to tag purple enemy squares before they run out of time.
When a purple enemy square runs out of time, you lose.";

HelpTime := 5;

ENEMY ::= (position: Point(0), remainingTime: int(0));

State ::= (time: int(0), highScore: int(0), playerPosition: Point(0), enemies: ENEMY(1), isDead: bool(0)); //Fill in this struct with the game state members.
...
initialState := (time: 0, highScore: 0, isDead: false, playerPosition: playerStart, enemies: []);
...
newState(I(0), S(0)) := 
	let
		...
		newPos := I.iClick.clPoint; //point(newX, newY);
		...
		enemyTicks[i] := tickEnemies[i].remainingTime;	
		
		newHighScore := max(S.highScore, S.time);
		
		died := S.isDead or some(enemyTicks < 0);
	in
		(time: 0, highScore: newHighScore, isDead: false, playerPosition: playerStart, enemies: []) when clickedReset(I, S)
	else
		S when S.isDead
	else
		(time: S.time + 1, highScore: newHighScore, isDead: died, playerPosition: newPos, enemies: makeNewEnemy(S) ++ tickEnemies);

clickedReset(I(0), S(0)) := 
	let
		buttonLeft := resetButton[1].center.x - resetButton[1].radius;
		buttonRight := resetButton[1].center.x + resetButton[1].radius;
		buttonTop := resetButton[1].center.y + resetButton[1].radius;
		buttonBottom := resetButton[1].center.y - resetButton[1].radius;
	in
		S.isDead and I.iClick.clicked and I.iClick.clPoint.x >= buttonLeft and I.iClick.clPoint.x <= buttonRight and I.iClick.clPoint.y >= buttonBottom and I.iClick.clPoint.y <= buttonTop;

sounds(I(0), S(0)) := 
	let
		resetSound := ["ding"] when clickedReset(I, S) else [];
	...
	in
		diedSound ++ resetSound ++ newEnemySound ++ killSound;

makeNewEnemy(S(0)) :=
	let
		r := randSeq(((S.time * 2000) + (S.playerPosition.x * S.playerPosition.y) + 1 + size(S.enemies) + S.highScore) * 33, 3);
...
resetButton := [disc(point(200, 600), 50, dRed), text("Click to\n Reset!", point(200, 600), 12, dWhite), circle(point(200, 600), 50, dBlack), circle(point(200, 600), 49, dBlack)];

images(S(0)) := 
	let
		time := text("Time: " ++ Conversion::intToString(S.time / 30), point(800, 750), 16, dBlue);
		score := text("High Score: " ++ Conversion::intToString(S.highScore / 30), point(800, 710), 16, dBlue);
		...
		help := [text(HelpMessage, point(500, 50), 16, dBlue)] when S.time < (30 * HelpTime) or S.isDead  else [];
	in
		join(drawEnemy(S.enemies)) ++ player ++ [time, score] ++ resetPic ++ help;
